...
def execute(month, year):
	...
	for document in documents :
		...
		if data:
			...
			entity_location = data[0]["entity"]["LOC"]
			entity_num = data[0]["entity"]["NUM"]
			...
			#get value unique
			arr_unique_kota = list(set(entity_location))
			arr_unique_num = list(set(entity_num))
			...
			temp_location = dbmodel.is_candidate_loc(db_location, location_collection, entity_position_from_db, entity_location)
			loc_clear = dbmodel.is_real_loc(db_location, location_collection, temp_location)

			res_unique_num = []
			for num in entity_num:
			...
			if loc_clear:
				...
				for real_loc in loc_clear:
					...
					if dbmodel.check_loc_and_date(fwc_db,month,real_loc,time):
						#maka jalankan statement berikut 
						district = dbmodel.district_check(db_location,location_collection,real_loc)
						...
						for num in res_unique_num:
							...
							for ind_loc in temp_loc:
								for ind_num in entity_position_from_db[num]:
									arr_index_num[num] = abs(ind_loc-ind_num)

						num_min = min(arr_index_num.items(), key=lambda x: x[1])
						loc_data["NUM"] = int(num_min[0])
						if dbmodel.check_num_in_loc_and_date(fwc_db,month,real_loc,num,time):
							#apabila data lokasi, angka kejadian dan hari sama, maka push url duplicate
							for url in url_duplicate:
								upd = dbmodel.update_data_fwc_push_url_duplicate(fwc_db,month,real_loc,num,time,url)
								print upd
						else:
							...
							ins = dbmodel.bulk_insert(fwc_db,month,loc_data)
							# print "lokasi : %s, jumlah penderita : %d"%(real_loc,loc_data["NUM"])
							print id_tweet
							# reset loc_data
							loc_data = {}
							loc_data["LOC"] = real_loc
						...
						if len(res_unique_num) > 1:
							...
							for num in res_unique_num:
								...
								for ind_loc in temp_loc:
									for ind_num in entity_position_from_db[num]:
										dist_num_to_loc = abs(ind_loc-ind_num)
										arr_index_num[num] = dist_num_to_loc

							# cari minimal jarak antara kota dengan angka kejadian 
							num_min = min(arr_index_num.items(), key=lambda x: x[1])
							loc_data["NUM"] = int(num_min[0])
							# print "lokasi : %s, jumla penderita : %d"%(loc_data["LOC"],loc_data["NUM"])
						else:
							loc_data["NUM"] = int(res_unique_num[0])
						...
						loc_data["url_duplicate"] = url_duplicate
						loc_data["time"] = time
						if not dbmodel.check_num_in_loc_and_date(fwc_db,month,real_loc,loc_data["NUM"],time):
							ins = dbmodel.bulk_insert(fwc_db,month,loc_data)
							# reset loc_data
							loc_data = {}
							print id_tweet
